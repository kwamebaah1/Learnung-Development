import { Search, LayoutDashboard, BellDot, Star, Truck, Users, Home, MessageSquare, Settings, Plus, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
const FleetSidebar = () => {
  ...
  const StatusIndicator = ({ status, hasAlert = false }: { status: string; hasAlert?: boolean }) => {
    if (hasAlert) {
      return (
  ...
  const DriverItem = ({ driver, hasAlert = false }: { driver: any; hasAlert?: boolean }) => (
  ...
  return (
    <div className="fixed left-0 top-0 h-screen flex z-50">
      ...
      <div className="w-16 bg-black h-full flex flex-col">
        {/* Navigation Icons , fleet-sidebar-dark*/}
        <div className="flex flex-col gap-2 p-4 border-b border-fleet-sidebar-muted">
          {navigationItems.map((item, index) => (
            <Button
              ...
              className={`w-10 h-10 p-0 ${
                item.active 
                  ? 'bg-fleet-sidebar-accent text-white' 
                  : 'text-fleet-sidebar-foreground hover:bg-fleet-sidebar-muted hover:text-white'
              }`}
            >
        ...
        <div className="mt-auto p-4 border-t border-fleet-sidebar-muted">
      ...
      {/* Content Section */}
      <div className="w-64 bg-white h-full flex flex-col border-l border-gray-200">
        {/* Search */}
